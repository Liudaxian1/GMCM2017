...
ylabel('最优值')
%}
[ObjV zhuangzai]=sumtime(Chrom,Shortest_Time_A,Shortest_Time_B,Shortest_Time_C,dis);  %计算路径长度
preObjV=min(ObjV);
while gen<MAXGEN
    %% 计算适应度
    [ObjV zhuangzai]=sumtime(Chrom,Shortest_Time_A,Shortest_Time_B,Shortest_Time_C,dis);  %计算路径时间
    % fprintf('%d   %1.10f\n',gen,min(ObjV))
    ...
    %% 逆转操作
    SelCh=Reverse(SelCh,Shortest_Time_A,Shortest_Time_B,Shortest_Time_C,dis);
    %% 重插入子代的新种群
...
disp(['总距离：',num2str(ObjV(minInd(1)))]);j
disp('-------------------------------------------------------------')
%}
