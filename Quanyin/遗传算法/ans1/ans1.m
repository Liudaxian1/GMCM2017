...
ylabel('最优值')
%}
[ObjV zhuangzai]=sumtime(Chrom,Shortest_Time_A,Shortest_Time_B,Shortest_Time_C);  %计算路径长度
preObjV=min(ObjV);
while gen<MAXGEN
    %% 计算适应度
    [ObjV zhuangzai]=sumtime(Chrom,Shortest_Time_A,Shortest_Time_B,Shortest_Time_C);  %计算路径时间
    % fprintf('%d   %1.10f\n',gen,min(ObjV))
