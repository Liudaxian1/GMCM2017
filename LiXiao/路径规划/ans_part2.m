...
tim(car,1:size_2(car,1)+size_3(car,1))=[tim_2(car,1:size_2(car,1)),tim_2(car,size_2(car,1))+1/6+tim_3(car,1:size_3(car,1))];
    ...
    while loc_2(car,j)~=(zpoint(car,1)+2)
        ...
        if car>=13
            ...
            num_z=number_z(i,zpoint(car,1)+2, tim_2(car,j),loc,tim,pri); %判断装载区域实时车辆
            delta_t=0;
            loc_2(car,j+1)=Path_dyn(1,2);
            tim_2(car,j+1)=tim_2(car,j)+Time_dyn(1,2);
            while num_z>=2
                tim_2(car,j+1)=tim_2(car,j+1)+1/60;
                num_z=number_z(i,zpoint(car,1)+2, tim_2(car,j+1),loc,tim,pri);
            end             
            j=j+1;
    end  
    ...
    tim_3(car,1:flag)=max(tim_3(find(pri==1),:))-max(tim_3(car,:))+tim_3(car,1:flag);
    t_adjust=tim_3(car,1)-tim_3(find(pri==1),1);
    j=1;
        ...
        if Path_dyn(1,2)~=Path(1,2)
            if t_adjust>=(Time_dyn(1,2)-Time(1,2))
                ...
                j=1;
                tim_3(car,1)=t_adjust+tim_3(car,1);
            else
